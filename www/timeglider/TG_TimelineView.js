...
tg.TG_TimelinePlayer = function (widget, mediator) {
	...
	*/
	...
	this._templates = {
		...
		event_modal_small: "<div class='tg-modal timeglider-ev-modalsmall ui-widget-content ${extra_class}' id='${id}_modal'>" 
            ...
            //For displaying the update option
    
       event_modal_view: "<div class='tg-modal timeglider-ev-modal ui-widget-content ${extra_class}' id='${id}_modal'>" 
      	   + "<div class='tg-close-button tg-close-button-remove'></div>" 
      	   + "<div class='dateline'>{{html dateline}}</div>"
           + "<form id ='form2' method='POST' action='C:\\Users\\maryum.babar\\ptimeglider\\www\\Edit_Task.php'>"
      	   + "<label id='title'>${title}</label>"
      	   + "<div class='tg-ev-modal-description jscroll'><p>{{html image}}{{html description}}</p></div>"
           + "<div class='tg-ev-modal-description'>"
                        +"<div style='display:inline-block;vertical-align: middle;'>"
                        +"<label id='l1'style='font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;margin:8px;'>start date:</label>"
                        +"<input name='start_date' id='end_date' type='date' style='border-radius: 5px'>"
                        +"<label style='font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;margin:8px;'>time:</label>"
                        +"<input name='start_time' id='end_time' type='time' style='border-radius: 5px'>"+"<br/>"
                        +"<label id='l1'style='font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;margin:8px;'>end date:</label>"
                        +"<input name='end_date' id='end_date' type='date' style='border-radius: 5px'>"
                        +"<label style='font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;margin:8px;'>time:</label>"
                        +"<input name='end_time' id='end_time' type='time' style='border-radius: 5px'>"+"<br/>"
                        +"<input type='submit' value='Update' style='background-color: #14B9D6;border-radius: 5px;font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;'>"
                        +"</div>"
                        +"<div style='margin-left:20px;padding:0px;display:inline-block;vertical-align: middle;'>"
                        +"<p style='margin:0px;width:80px;background-color: white ;padding:0px;font: normal normal 15px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;'>Comments</p>"
                        +"<textarea name='comments' id ='description' type='text' style='text-align: start ;border-color:transparent ;width:350px;height:80px;'></textarea>"
                        +"</div>"
           ...
           event_modal_small_update: "<div class='tg-modal timeglider-ev-modal ui-widget-content ${extra_class}' id='${id}_modal'>" 
      	   + "<div class='tg-close-button tg-close-button-remove'></div>" 
      	   + "<div class='dateline'>{{html dateline}}</div>"
           + "<ul style='color:#a3daf5;margin:0px;display:block;vertical-align:baseline;list-style:none'>"
           + "<li class='tab1' style='float:left;margin:0px;width:100px;background-color:#14b9d6;padding:5px;font: normal normal 15px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;'>Task Details</li>"
           + "<li class='tab2' style='float:left;margin:0px;width:100px;background-color:#14b9d6 ;padding:5px;font: normal normal 15px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;'>Task History</li>"
           
           + "</ul>"
           + "<img id='add_sub_task' src='timeglider/img/plus_strong_cyan.png'>"
           + "<br/>"
           + "<div id = 'tab-container' style='padding-top:10px;margin-top:10px;margin-left:40px;border-color:#14b9d6;border-style:solid;border-width:thin;width:450px;height:250px;' >"
          
            + "<div id='task_details' >"
             + "<label id='title'>${title}</label>"
             + "<div class='tg-ev-modal-description jscroll'><p>{{html image}}{{html description}}</p></div>"
                 + "<div class='tg-ev-modal-description'>"
                      +"<label id='l4'style='font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;margin:8px;'>status:</label>"
                      +"<label id='l4'style='font: normal normal 5px 'Dancing Script','Segoe UI',sans-serif;color:#14B9D6;margin:8px;'>${status}</label>"
                      +"<br/>"
                      +"<input id = 'taskid' type = 'hidden' name='taskid' value='${id}'>"
                 + "</div>"
            + "</div>"
            +"<div id='history' style='display:none;'>"
                 +"<table id='task_history' border='1' style='color:#14b9d6;font-size: 12px;font-weight:bold;border-color: #14b9d6; '>"
                 +"<tr>"
                 +"<th>Start Date</th>"
                 +"<th>End Date</th>"
                 +"<th>Comment</th>"
	...
	this.timelineInfoModal = Backbone.View.extend({
		...
		template: function () {
			...
			return "<div class='tg-modal-corner tg-modal-corner-north'></div>"
                                +"<h4>${title}</h4>"
	...
	$(CONTAINER)
		...
		.delegate(".tg-legend-none", CLICKORTOUCH, function () {
			...
			MED.setFilters({origin:"legend", icon: "none"});
                ...
                .delegate(".tg-timeline-new", CLICKORTOUCH, function() {
			var ptitle = $('#project-title').text();
                        
                        $('#dialog-form').load("Create_New_Event.php",{proj_title:ptitle});
...
tg.TG_TimelinePlayer.prototype = {
	...
	initiateNavigation: function() {
		...
		$(".tg-single-timeline-header .tg-timeline-start").fadeIn();
                $(".tg-single-timeline-header .tg-timeline-new").fadeIn();
  ...
  registerTitles: function () {
	...
	setupSingleTimeline: function() {
		...
		if (MED.options.display_single_timeline_info != false) {
			...
			var title = "<h2 id='project-title'>" + timeline.get("title") + "</h2>";
			...
			tmpl = "<div class='tg-widget-header tg-single-timeline-header'>" + title + "<ul>" + inf + leg +"<li id = 'timeline-new-event' click='createNewEvent' class='tg-timeline-new' data-timeline_id'"+tid+"'>Create New Event</li>" +"<li class='tg-timeline-start' data-timeline_id='" + tid + "'>Start</li></ul>" + tools + "</div>";
		...
		var me = this,
			...
			// if the embed size is small
				...
				modal_type = "full";
                            ...
                            //{
                                //modal_type = "event_modal_small_update";
                            //}
						// return false;
                        modal_type = "event_modal_small_update";
			...
			switch (modal_type) {
				...
				case "event_modal_small_update":
                                    
                                    templ_obj.extra_class = (templ_obj.image) ? "has-image":"no-image";
										
   					$modal = $.tmpl(me._templates.event_modal_small_update,templ_obj).appendTo($event.parent());
					...
					if (ev_rel < pad) {
						...
						lr_class = "left";
                                        ...
                                        $('#add_sub_task').click(function(){
                                                ...
                                                $('#dialog-form').load("Add_New_Sub_Task.php",{taskid:task_id});
                                        ...
                                        $.ajax({
                                        type: "POST",
                                        url: "View_Task_History.php?taskid="+task_id,
                                        data: task_id ,
                                        success: function(data) {
                                            ...
                                            var row = '';
                                            for (var i in history.rows) {
                                                row += '<tr id="' + i + '">';
                                                row += '<td>' + history.rows[i].startdate + '</td>';
                                                console.log(history.rows[i].startdate);
                                                row += '<td>' + history.rows[i].enddate + '</td>';
                                                console.log(history.rows[i].enddate);
                                                row += '<td>' + history.rows[i].comments + '</td>';
                                                console.log(history.rows[i].comments);
                                                row += '<td>' + history.rows[i].user + '</td>';
                                                console.log(history.rows[i].user);
                                                row += '</tr>';
                                            }
                                            $(row).appendTo('#task_history');
                                            /*for(var i=0;i<history.length;i++)
                                            {
                                                //console.log("i ->" + i);
                                                $("<tr></tr>").attr("id",i).appendTo('#task_history');
                                                for (var j=0;j<history[i].length;j++)
                                                {
                                                    
                                                    $("<td></td>").appendTo('#'+i).text(history[i][j]);
                                        ...
                                        //$('#history').load('C:\\Users\\maryum.babar\\ptimeglider\\www\\View_Task_History.php');
                                        $('.tab1').click(function(){
                                           ...
                                           $('.tab2').css({'background-color': '#14b9d6','border': 'none','color': '#a3daf5'});
                                        });
                                        
                                        $('.tab2').click(function(){
                                           $('.tab2').css({'background-color': '#a3daf5','border-style': 'solid','border-color': '#14B9D6','border-width':'thin','color': '#14B9D6'});
                                           $('.tab1').css({'background-color': '#14b9d6','border': 'none','color': '#a3daf5'});
		...
		if (this.titleBar == "fullBar") {
			...
			tmpl = "<div class='titleBar'>"
	      	...
	      	if (this.model.get("description")) {
                ...
                //CREATE NEW EVENT
                if (this.model.get("description")) {
	      		env_bts += "<div class='tg-env-button tg-env-info tg-timeline-new' data-timeline_id='${id}'></div>";
	      	}
